...
while(:;) do
	...
	#Inicio Firewall
		...
		sudo iptables -t nat -A POSTROUTING -s $system_ip/24 -j MASQUERADE
		echo "1" >/proc/sys/net/ipv4/ip_forward
		dialog --title 'AVISO' --infobox 'O sistema está habilitado como servidor' 0 0
		sleep 5
		while(:;) do
		REGRA=$(dialog --stdout --nocancel --title 'Menu' --menu 'O que deseja Fazer' 0 0 0 1 "Ver Regras Habilitadas" 2 "Remover todas as Regras INCLUINDO NAT" 3 "Bloquear IP" 4 "Roteamento" 0 "Sair")
		case "$REGRA" in
		0)
			break;;
		1)
			...
			echo -e "-------------------------------------------------------------------------" >> info.txt
			sudo iptables -L >> info.txt
			echo -e "-------------------------------------------------------------------------" >> info.txt
			echo -e "Regras implementadas na tabela NAT:" >> info.txt
			echo -e "-------------------------------------------------------------------------" >> info.txt
			sudo iptables -L -n -t  nat >> info.txt
			echo -e "-------------------------------------------------------------------------" >> info.txt
		...
		3)
			...
			sudo iptables -I INPUT -s $blocked_ip -j DROP
			;;
		4)	
			...
			1)
				...
				sudo iptables -t nat -A PREROUTING -s $source_ip -i $select_interface -j DNAT --to $destiny_ip
				;;
			2)	
				...
				dialog --title 'Interfaces disponiveis para seleção' --textbox info.txt 0 0
                select_interface=$(dialog --inputbox --stdout 'Digite a interface de rede a ser utilizada:' 0 0 )
                sudo iptables -t nat -A POSTROUTING -s $source_ip -o $select_interface -j SNAT --to $modified_ip
                ;;
			...
			3)
				...
				sudo iptables -A FORWARD -p tcp -d $source_ip --dport $port -j ACCEPT
				;;
			esac
			;;
		esac
